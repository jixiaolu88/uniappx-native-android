...
open class GenUniModulesTmxUiComponentsXFormXForm : VueComponent {
    ...
    override fun data(): Map<String, Any?> {
        return utsMapOf("list" to utsArrayOf<FORM_ITEM>(), "top" to 0, "isOkTop" to false, "oldFormData" to UTSJSONObject() as Any, "isdestry" to false, "_modelValue" to computed<Any>(fun(): Any {
            return this.modelValue;
        }
        ));
    }
        ...
        this._valid = fun(keys: UTSArray<String>): FORM_SUBMIT_RESULT {
            ...
            if (this.errorAutoPage && !result.valid && top > 0) {
                uni_pageScrollTo(PageScrollToOptions(scrollTop = top, fail = fun(_){
                    console.error("当前页面没有放置可滚动的列表，但使用了表单出错滚动功能，请关闭表单属性：error-auto-page", " at uni_modules/tmx-ui/components/x-form/x-form.uvue:246");
                }
                ));
            }
            return result;
