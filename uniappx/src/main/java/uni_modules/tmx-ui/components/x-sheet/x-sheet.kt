...
open class GenUniModulesTmxUiComponentsXSheetXSheet : VueComponent {
        ...
        return createElementVNode("view", utsMapOf("onClick" to _ctx.onclick, "onTouchend" to _ctx.mend, "onTouchcancel" to _ctx.mendcel, "onTouchstart" to _ctx.mst, "class" to "xSheet", "style" to normalizeStyle(_ctx._styleMap)), utsArrayOf(
            if (isTrue(_ctx._loading)) {
                createElementVNode("view", utsMapOf("key" to 0, "onClick" to withModifiers(fun(){}, utsArrayOf(
                    ...
                    createVNode(_component_x_icon, utsMapOf("spin" to true, "font-size" to "42", "name" to "loader-3-line", "color" to _ctx._loadingColor), null, 8, utsArrayOf(
                        "color"
                    ))
                ), 12, utsArrayOf(
                    "onClick"
        ...
        this.onclick = fun() {
            if (this.url != "") {
                uni_navigateTo(NavigateToOptions(url = this.url, fail = fun(error) {
                    console.error(error, " at uni_modules/tmx-ui/components/x-sheet/x-sheet.uvue:326");
                }
                ));
                return;
            }
    ...
    open lateinit var mst: () -> Unit;
    open lateinit var mend: () -> Unit;
    open lateinit var mendcel: () -> Unit;
    companion object {
        ...
        , "validator" to fun(kVal: UTSArray<String>): Boolean {
            ...
            if (kVal.length != 3) {
                console.error("x:渐变属性linearGradient可以为空数组，提供具体值是必须长度为3", " at uni_modules/tmx-ui/components/x-sheet/x-sheet.uvue:83");
                return false;
            }
            return true;
        }
