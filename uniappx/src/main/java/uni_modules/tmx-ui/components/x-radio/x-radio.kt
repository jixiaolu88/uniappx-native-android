...
open class GenUniModulesTmxUiComponentsXRadioXRadio : VueComponent {
    ...
    override fun data(): Map<String, Any?> {
        return utsMapOf("nowValue" to "0", "boxId" to ("xRadio-" + getUid()), "tid" to 0, "isDestroy" to false, "_color" to computed<String>(fun(): String {
            if (this.color == "") {
    ...
    override fun `$initMethods`() {
        ...
        ;
        this.setSelected = fun(kVal: String) {
        ...
        this.pushDataToParent = fun(isChange: Boolean) {
            var parent: GenUniModulesTmxUiComponentsXRadioGroupXRadioGroupComponentPublicInstance? = null;
            try {
                parent = this.`$parent` as GenUniModulesTmxUiComponentsXRadioGroupXRadioGroupComponentPublicInstance?;
            }
             catch (e: Throwable) {}
            if (parent == null) {
                return;
            }
            var parents = parent as GenUniModulesTmxUiComponentsXRadioGroupXRadioGroupComponentPublicInstance;
            parents.addItem(this as GenUniModulesTmxUiComponentsXRadioXRadioComponentPublicInstance, RADIO_ITEM_INFO(id = this.boxId as String, nowvalue = this.nowValue, value = this.value, unvalue = this.unCheckValue), isChange);
        }
        ;
    }
    open lateinit var boxClick: () -> Unit;
    open lateinit var setAni: () -> Unit;
    open lateinit var setSelected: (kVal: String) -> Unit;
